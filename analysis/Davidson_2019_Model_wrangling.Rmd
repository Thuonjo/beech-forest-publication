...
```{r sorting-references, eval=FALSE, include=FALSE}
library(citr)
...
This report investigates the estimation of the rates of change between seasons. The data is in a similar structure as historically used for estimation of numerical and functional responses.

# Introduction

*Generally look at thesis drafts [here]("https://www.ssnhub.com/").*

These files are the supporting documetation to the data-wrangling code in the Beech forest manuscript [here]("https://www.ssnhub.com/invasive-species-research.html/").

But there is also many other supporting resources and less formal tutorials on my [website](https://www.ssnhub.com).

#  Methods

This report breaks down the parameter estimates from the bayesian hierarical model in the context of our predictions and the data we collected.

## Data

First the data for each parameter needs to be extracted and plotted. It can be hard to extract the information needed from a Bayesian output so I have left this for another vignette. For now I have just extracted the needed inforamtion for this paper.

*I Have used the following two datasets generated from the `Davidson_2019_Data_warngling.RMD` file*

```{r import-data-previous-data}
# Overall Data
...
- valley = `egl` and `hol`
- Valley = `eglinton` and `hollyford`
- Control = 
- Control = 
- trip = trip.no
- true.date = Date
- var = raw data parameter

### Reduce dataset

```{r reduce-previous-data}
...
#### Extracting rates from model outputs
...
##### Reconstructing parameter dataset
...
glimpse(para.plot.dat)
```


### Output datasets for modelling
- Parameter estimates: *n =* **48**
- Rate of change: *n =* **136**
- Abundance *n =* **144**

# Plots

Overall there are many combinations of plots that can be produced from a bayesian hierarichal model.
...
This is where it gets really tricky. I have done this in a way that works. I am sure there are better ways to code this but that is for a later date.

My steps are:

1. `out.final1` and filter out only month of interest
2. Find max values
- background points
2. `spread` data across the different parameter estimates
4. reduce the `out.full.136` to only `feb`
3. Merge with parameter dataset (steps1-2)
4. Plot away using `ggplot2`

## Seed

### All months

This is ok becuase the average estimates of line will all use the same average to estimate the trend line for each parameter and variable.

It comes difficult to combine plots when we want to compare differences between varaibles and rate of increase for a fixed time (month = ``).
...
This phase (Autumn to winter) has the strongest effect of `intake rate` on the population growth rates ($r_{jt}$).
...
# para.plot.all <- 
  ...
  theme(strip.background = element_blank(),
        ...
        axis.title.y = element_text(colour = "black",size =10, family = "Times", angle = 90),
        axis.title.x = element_text(colour = "black", size =10, family = "Times"),
...
## Prediction C: Autumn and Winter trends
...
## Prediction D: Spring and Summer trends
...
# Save plot
# export plot for example vignette
png("../figs/fig-7-1.png",width = 6, height = 5)
para.plot.rat
dev.off()
