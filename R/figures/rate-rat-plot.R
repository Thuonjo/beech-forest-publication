...
pE.plot.3 <- out.full.136 %>%
  filter(month == "Aug")

pe.plot.feb <- ggplot(pE.plot.3, aes(y = mean.r, x = lag.rat.mna)) +
  ...
  geom_point(data = out.rat.feb2, aes(y = mean.r, x = lag.sjt), size = 3) +
  geom_line(data = out.rat.feb2, aes(y = mean.r, x = lag.sjt, group = treat),size = 1) +
...
# xlab(expression(paste("Intake"," ", "Rate"," ","(",italic(S[jt]),")" ))) +
 ...
 xlab(expression(atop(paste("Minimum"," ", "number"," "),paste("of", " ", "rats"," ","(",italic(R[j,t-1]),")"))) )+
